...
import { CreatePool } from '../types/StreamSwapStream.types';
...
const simulation_1 = async () => {
    ...
    // Simulation 1: 
    // Create a stream with pool

    // Details:
    // 1. Create a stream with pool
    // 1.1. Pool out amount is the half of the total amount.
    ...
    // 5. Subscibe to the stream at bootstraping. <Subsriber2>
    ...
    // 9. Creator finalizes the stream after it ends.
    ...
    // Half of the stream out amount
    let poolOutAmount = 500000000000000;

    let totalAmount = (streamAmount + poolOutAmount).toString();
    ...
    await controller.CreateStream(context, out_asset, deploymentConfig.in_denom, startTime.toString(), endTime.toString(), bootstrapingStartTime.toString(), { with_pool: true, pool_out_amount_clp: poolOutAmount.toString() });
    ...
    await stream.SubscribeToStream(context, { denom: deploymentConfig.in_denom, amount: '10000000' }, 'subscriber_2');

    // Await until start+60 seconds
    await sleepUntil(nanoToDate(startTime + 60));

    // Withdraw half amount from the stream
    await stream.WithdrawFromStream(context, '5000000', 'subscriber_2');
    ...
    // Pool configuration 
    let poolCreationConfig = deploymentConfig.pool_config;

    // Creator finalizes the stream
    await stream.FinalizeStream(context, undefined, poolCreationConfig);
    ...
    logger.log(1, 'Simulation 1 completed');
